...
//					ArrayList<MKCityListInfo> enCities = res.getAddrResult().mEndCityList;
	...
	void SearchButtonProcess(View v) {
		...
		EditText editSc = (EditText)findViewById(R.id.citys);
		EditText editEc = (EditText)findViewById(R.id.citye);
        ...
        // 添加起点终点城市
		String s1=  editSc.getText().toString();
		String s2=  editEc.getText().toString();
		...
		if (mBtnDrive.equals(v)) {
			...
			/*for (int i =0;i<3;i++)
			{
				if(i==0)
				{
					mSearch.setDrivingPolicy(MKSearch.ECAR_TIME_FIRST);//驾车策略:时间优先、费用最少或距离最短
					mSearch.drivingSearch(s1, stNode, s2, enNode);
				}
				if(i==1)
				{
					mSearch.setDrivingPolicy(MKSearch. ECAR_DIS_FIRST);//驾车策略:时间优先、费用最少或距离最短
					mSearch.drivingSearch(s1, stNode, s2, enNode);
				}
				if(i==2)
				{
					mSearch.setDrivingPolicy(MKSearch.ECAR_FEE_FIRST);//驾车策略:时间优先、费用最少或距离最短
					mSearch.drivingSearch(s1, stNode, s2, enNode);
				}
